                ...
                if (!statusCode || statusCode === 400) {
                    if (this.showText) {
                        this.scene.ui.showText(
                            i18next.t("menu:logInOrCreateAccount")
                        );
                    }
    ...
    loadSaveSlot(slotId: integer): void {
        ...
        this.scene.gameData
            ...
            .catch((err) => {
                console.error(err);
                this.scene.ui.showText(
                    i18next.t("menu:failedToLoadSession"),
                    null
                );
            });
    }
    ...
    initDailyRun(): void {
        this.scene.ui.setMode(
            ...
            (slotId: integer) => {
                ...
                if (!Utils.isLocal) {
                    ...
                    generateDaily(
                        btoa(new Date().toISOString().substring(0, 10))
                    );
                }
            }
        );
        ...
        if (this.loaded) {
            ...
            if (this.scene.currentBattle.double && availablePartyMembers > 1) {
                this.scene.pushPhase(
                    new SummonPhase(this.scene, 1, true, true)
                );
            }
    ...
    start() {
        ...
        battle.enemyLevels.forEach((level, e) => {
            if (!this.loaded) {
                if (battle.battleType === BattleType.TRAINER) {
                    ...
                    if (
                        this.scene.currentBattle.battleSpec ===
            ...
            if (e < (battle.double ? 2 : 1)) {
                enemyPokemon.setX(
                    -66 + enemyPokemon.getFieldPositionOffset()[0]
                );
        ...
        if (battle.battleType === BattleType.TRAINER) {
            loadEnemyAssets.push(
                battle.trainer
                    .loadAssets()
                    .then(() => battle.trainer.initSprite())
            );
        } else {
                ...
                if (!this.loaded) {
                    this.scene.gameData
                        .saveAll(
                            this.scene,
                            true,
                            battle.waveIndex % 10 === 1 ||
                                this.scene.lastSavePlayTime >= 300
                        )
                        .then((success) => {
    ...
    doEncounterCommon(showEncounterMessage: boolean = true) {
        ...
        if (this.scene.currentBattle.battleType === BattleType.WILD) {
            ...
            const doSummon = () => {
                ...
                const doTrainerSummon = () => {
                    ...
                    this.scene.unshiftPhase(
                        new SummonPhase(this.scene, 0, false)
                    );
                    if (
                        this.scene.currentBattle.double &&
                        ...
                        this.scene.unshiftPhase(
                            new SummonPhase(this.scene, 1, false)
                        );
                    }
                    this.end();
                };
                ...
                this.scene.pushConditionalPhase(
                    ...
                    () => {
                        ...
                        const requiredPokemonsOnField = Math.min(
                            this.scene.getParty().filter((p) => !p.isFainted())
                                .length,
                            2
                        );
                        ...
                        if (this.scene.currentBattle.double) {
                            return (
                                pokemonsOnFieldCount === requiredPokemonsOnField
                            );
                        }
        ...
        if (!this.loaded) {
            ...
            if (this.scene.currentBattle.double) {
                ...
                this.scene.pushPhase(
                    new ToggleDoublePositionPhase(this.scene, false)
                );
            }
    ...
    doEncounter(): void {
        ...
        this.scene.tweens.add({
            ...
            onComplete: () => {
                ...
                this.scene.arenaNextEnemy.setX(
                    this.scene.arenaNextEnemy.x - 300
                );
...
export class SelectBiomePhase extends BattlePhase {
    ...
    start() {
        ...
        const setNextBiome = (nextBiome: Biome) => {
            ...
            this.scene.unshiftPhase(
                new SwitchBiomePhase(this.scene, nextBiome)
            );
            this.end();
        };
        ...
        if (
            ...
            this.scene.executeWithSeedOffset(() => {
                biomes = (
                    ...
                    .filter(
                        (b) => !Array.isArray(b) || !Utils.randSeedInt(b[1])
                    )
            ...
            if (
                ...
                this.scene.executeWithSeedOffset(() => {
                    biomeChoices = (
                        ...
                        .filter(
                            (b, i) =>
                                !Array.isArray(b) || !Utils.randSeedInt(b[1])
                        )
    ...
    generateNextBiome(): Biome {
        ...
        return this.scene.generateRandomBiome(
            this.scene.currentBattle.waveIndex
        );
    }
}
        ...
        this.scene.tweens.add({
            ...
            onComplete: () => {
                ...
                this.scene.tweens.add({
                    ...
                    ease: "Sine.easeInOut",
                    alpha: (target: any) =>
                        target === this.scene.arenaPlayer ? 0 : 1,
                    onComplete: () => {
                        this.scene.arenaBg.setTexture(bgTexture);
            ...
            onComplete: () => {
                this.scene.tweens.add({
                    ...
                    onComplete: () => {
                        ...
                        if (!this.player) {
                            const playerPokemon =
                                this.scene.getPlayerPokemon() as Pokemon;
                            if (playerPokemon?.visible) {
                                this.scene.field.moveBelow(
                                    pokemon,
                                    playerPokemon
                            ...
                            this.scene.currentBattle.seenEnemyPartyMemberIds.add(
                                pokemon.id
                            );
                        }
                        addPokeballOpenParticles(
...
export class SwitchSummonPhase extends SummonPhase {
    ...
    preSummon(): void {
        if (!this.player) {
            if (this.slotIndex === -1) {
                this.slotIndex =
                    this.scene.currentBattle.trainer.getNextSummonIndex(
                        !this.fieldIndex
                            ? TrainerSlot.TRAINER
                            : TrainerSlot.TRAINER_PARTNER
                    );
        ...
        pokemon.hideInfo();
        pokemon.tint(
            getPokeballTintColor(pokemon.pokeball),
            1,
            250,
            "Sine.easeIn"
        );
            ...
            playerPokemon
                .setFieldPosition(
                    this.double &&
                        this.scene
                            .getParty()
                            .filter((p) => p.isAllowedInBattle()).length > 1
                        ? FieldPosition.LEFT
                        : FieldPosition.CENTER,
                    500
                )
    ...
    start() {
        ...
        this.scene.ui.showText(
            i18next.t("battle:switchQuestion", {
                pokemonName: this.useName
                    ? pokemon.name
                    : i18next.t("battle:pokemon"),
            }),
            null,
            () => {
                this.scene.ui.setMode(
...
export class TurnInitPhase extends FieldPhase {
    ...
    start() {
        ...
        this.scene.getPlayerField().forEach((p) => {
            ...
            if (p.isOnField() && !p.isAllowedInBattle()) {
                this.scene.queueMessage(
                    i18next.t("challenges:illegalEvolution", {
                        pokemon: p.name,
                    }),
                    null,
                    true
                ...
                if (!allowedPokemon.length) {
                    ...
                    allowedPokemon.length >=
                        this.scene.currentBattle.getBattlerCount() ||
                    (this.scene.currentBattle.double &&
                        !allowedPokemon[0].isActive(true))
                ) {
        ...
        this.scene.getField().forEach((pokemon, i) => {
            if (pokemon?.isActive()) {
                if (pokemon.isPlayer()) {
                    this.scene.currentBattle.addParticipant(
...
export class CommandPhase extends FieldPhase {
    ...
    start() {
        ...
        if (this.fieldIndex) {
            ...
            if (
                this.scene.getPlayerField().filter((p) => p.isActive())
                    .length === 1
            ) {
                ...
                if (
                    ...
                    if (
                        ...
                        turnCommand.move.targets =
                            playerPokemon.getMoveQueue()[0].targets;
                    } else {
                        this.scene.unshiftPhase(
                ...
                if (
                    ...
                    (!this.scene.gameMode.isClassic ||
                        (this.scene
                            ...
                            .some(
                                (p) =>
                                    !p.scene.gameData.dexData[
                                        p.species.speciesId
                                    ].caughtAttr
                            ) &&
                            this.scene.gameData.getStarterCount(
                                (d) => !!d.caughtAttr
                            ) <
                ...
                if (!isSwitch && this.scene.arena.biomeType === Biome.END) {
                    ...
                    if (batonPass || (!trapTag && !trapped.value)) {
                        ...
                        if (
                            ...
                            this.scene
                                .getPokemonById(trapTag.sourceId)
                                .isOfType(Type.GHOST)
                        ) {
                            success = true;
                            this.scene.currentBattle.turnCommands[
                        ...
                        this.scene.ui.showText(
                            i18next.t("battle:noEscapePokemon", {
                                pokemonName: this.scene.getPokemonById(
                                    trapTag.sourceId
                                ).name,
        ...
        if (trainer && !enemyPokemon.getMoveQueue().length) {
            ...
            if (!trapTag && !trapped.value) {
                ...
                if (partyMemberScores.length) {
                    ...
                    const matchupScore =
                        matchupScores.reduce(
                            (total, score) => (total += score),
                            0
                        ) / matchupScores.length;

                    const sortedPartyMemberScores =
                        trainer.getSortedPartyMemberMatchupScores(
                            partyMemberScores
                        );
    ...
    start() {
            ...
            if (aCommand.command !== bCommand.command) {
                ...
                applyMoveAttrs(
                    ...
                    this.scene
                        .getField()
                        .find(
                            (p) => p?.isActive() && p.getBattlerIndex() === a
                        ),
                    null,
                    aMove,
                    aPriority
                ...
                applyMoveAttrs(
                    ...
                    this.scene
                        .getField()
                        .find(
                            (p) => p?.isActive() && p.getBattlerIndex() === b
                        ),
                    null,
                    bMove,
                    bPriority
            ...
            switch (turnCommand.command) {
                case Command.FIGHT:
                    ...
                    const move =
                        pokemon
                            .getMoveset()
                            .find((m) => m.moveId === queuedMove.move) ||
        ...
        for (const o of order) {
            if (field[o].status && field[o].status.isPostTurn()) {
                this.scene.pushPhase(
                    new PostTurnStatusEffectPhase(this.scene, o)
                );
            }
        }
                ...
                this.scene.pushPhase(
                    new MessagePhase(
                        ...
                        i18next.t("battle:notDisabled", {
                            ...
                            moveName:
                                allMoves[pokemon.summonData.disabledMove].name,
                        })
                    )
                );
            ...
            new MoveAnim(
                ...
                for (const target of targets) {
                    ...
                    const firstHit =
                        user.turnData.hitsLeft === user.turnData.hitCount;
                    const firstTarget =
                        moveHistoryEntry.result === MoveResult.PENDING;
                                        ...
                                        if (hitResult !== HitResult.NO_EFFECT) {
                                            applyFilteredMoveAttrs(
                                                ...
                                                if (
                                                    hitResult <
                                                        HitResult.NO_EFFECT &&
                                                    !target.hasAbilityWithAttr(
                                                        IgnoreMoveEffectsAbAttr
                                                    )
                                                ) {
                                                    const flinched =
                                                        new Utils.BooleanHolder(
                                                            false
                                                        );
                ...
                if (!!postTarget) {
                    if (applyAttrs.length) {
                        ...
                        applyAttrs[applyAttrs.length - 1]?.then(
                            () => postTarget
                        );
                    } else {
        ...
        if (user) {
            if (--user.turnData.hitsLeft >= 1 && this.getTarget()?.isActive()) {
                ...
                const hitsTotal =
                    ...
                    Math.max(user.turnData.hitsLeft, 0);
                if (hitsTotal > 1) {
                    this.scene.queueMessage(
                        i18next.t("battle:attackHitsCount", {
                            count: hitsTotal,
                        })
                    );
                }
        ...
        if (userAccuracyLevel.value !== targetEvasionLevel.value) {
            accuracyMultiplier.value =
                userAccuracyLevel.value > targetEvasionLevel.value
                    ? (3 +
                          Math.min(
                              userAccuracyLevel.value -
                                  targetEvasionLevel.value,
                              6
                          )) /
                      3
                    : 3 /
    ...
    start() {
        ...
        const relLevels = filteredStats.map(
            (stat) =>
                (levels.value >= 1
                    ...
                    : Math.max(battleStats[stat] + levels.value, -6)) -
                battleStats[stat]
        );
        ...
        const end = () => {
            ...
            for (const stat of filteredStats) {
                pokemon.summonData.battleStats[stat] = Math.max(
                    Math.min(
                        pokemon.summonData.battleStats[stat] + levels.value,
                        6
                    ),
                    -6
                );
            ...
            handleTutorial(this.scene, Tutorial.Stat_Change).then(() =>
                super.end()
            );
        };
        ...
        if (relLevels.filter((l) => l).length && this.scene.moveAnimations) {
            ...
            const tileY =
                ...
                this.scene.field.scale;
            const tileWidth =
                156 * this.scene.field.scale * pokemon.getSpriteScale();
            const tileHeight =
            ...
            statSprite.setMask(
                new Phaser.Display.Masks.BitmapMask(
                    this.scene,
                    pokemonMaskSprite
                )
            );
        ...
        while (
            (existingPhase = this.scene.findPhase(
                (p) =>
                    ...
                    p.battlerIndex === this.battlerIndex &&
                    p.selfTarget === this.selfTarget &&
                    [BattleStat.ACC, BattleStat.EVA].some((s) =>
                        p.stats.includes(s)
                    ) === isAccEva &&
            ...
            if (!cancelled.value) {
                ...
                this.executeForAll((pokemon: Pokemon) => {
                    const immune =
                        ...
                        !!pokemon
                            .getTypes(true, true)
                            .filter((t) => this.weather.isTypeDamageImmune(t))
                            .length;
                    if (!immune) {
                    ...
                    this.scene.ui.showDialogue(
                        ...
                        () => {
                            this.scene.addEnemyModifier(
                                ...
                                ).newModifier(pokemon) as PersistentModifier,
                                false,
                                true
                            );
    ...
    doFaint(): void {
        ...
        if (pokemon.turnData?.attacksReceived?.length) {
            ...
            if (defeatSource?.isOnField()) {
                ...
                const pvmove =
                    allMoves[pokemon.turnData.attacksReceived[0].move];
                const pvattrs = pvmove.getAttrs(PostVictoryStatChangeAttr);
                if (pvattrs.length) {
                    for (const pvattr of pvattrs) {
                        pvattr.applyPostVictory(
                            defeatSource,
                            defeatSource,
                            pvmove
                        );
        ...
        if (this.player) {
            const nonFaintedLegalPartyMembers = this.scene
                .getParty()
                .filter((p) => p.isAllowedInBattle());
            const nonFaintedPartyMemberCount =
                nonFaintedLegalPartyMembers.length;
            ...
            this.scene.unshiftPhase(
                new VictoryPhase(this.scene, this.battlerIndex)
            );
            if (this.scene.currentBattle.battleType === BattleType.TRAINER) {
        ...
        if (this.scene.currentBattle.double) {
            ...
            if (allyPokemon?.isActive(true)) {
                ...
                do {
                    ...
                    if (
                        targetingMovePhase &&
                        targetingMovePhase.targets[0] !==
                            allyPokemon.getBattlerIndex()
                    ) {
                        targetingMovePhase.targets[0] =
                            allyPokemon.getBattlerIndex();
                    }
            ...
            this.scene.tweens.add({
                ...
                onComplete: () => {
                    ...
                    if (pokemon.isPlayer()) {
                        ...
                        this.scene.addFaintedEnemyScore(
                            pokemon as EnemyPokemon
                        );
                        this.scene.currentBattle.addPostBattleLoot(
                            pokemon as EnemyPokemon
                        );
                    }
                    this.scene.field.remove(pokemon);
        ...
        switch (this.scene.currentBattle.battleSpec) {
            case BattleSpec.FINAL_BOSS:
                if (!this.player) {
                    ...
                    if (enemy.formIndex) {
                        this.scene.ui.showDialogue(
                            battleSpecDialogue[BattleSpec.FINAL_BOSS]
                                .secondStageWin,
                            enemy.species.name,
                            null,
                            () => this.doFaint()
    ...
    start() {
        ...
        if (participantIds.size) {
            ...
            for (const partyMember of nonFaintedPartyMembers) {
                ...
                if (participated) {
                    ...
                    if (
                        ...
                        expMultiplier +=
                            multipleParticipantExpBonusModifier.getStackCount() *
                            0.2;
                    }
                } else if (expShareModifier) {
                    expMultiplier +=
                        (expShareModifier.getStackCount() * 0.2) /
                        participantIds.size;
                }
            ...
            if (expBalanceModifier) {
                ...
                const medianLevel = Math.floor(
                    totalLevel / expPartyMembers.length
                );
                ...
                expPartyMembers.forEach((_partyMember, pm) => {
                    partyMemberExp[pm] = Phaser.Math.Linear(
                        partyMemberExp[pm],
                        recipientExpPartyMemberIndexes.indexOf(pm) > -1
                            ? splitExp
                            : 0,
                ...
                if (this.scene.currentBattle.waveIndex % 10) {
                    ...
                    if (
                        ...
                        (this.scene.currentBattle.waveIndex <= 500 ||
                            this.scene.currentBattle.waveIndex % 30 ===
                                superExpWave)
                    ) {
                        this.scene.pushPhase(
                            new ModifierRewardPhase(
...
export class MoneyRewardPhase extends BattlePhase {
    ...
    start() {
        ...
        const formattedMoneyAmount =
            moneyAmount.value.toLocaleString(userLocale);
                        ...
                        () => {
                            this.scene.ui.fadeOut(1250).then(() => {
                                ...
                                this.scene.gameData
                                    ...
                                    .then(() => {
                                        this.scene.pushPhase(
                                            new EncounterPhase(this.scene, true)
                                        );
        ...
        const doGameOver = (newClear: boolean) => {
            ...
            this.scene.time.delayedCall(1000, () => {
                ...
                if (this.victory && newClear) {
                    if (this.scene.gameMode.isClassic) {
                        firstClear = this.scene.validateAchv(
...
export class SwitchPhase extends BattlePhase {
    ...
    start() {
        ...
        if (
            ...
            !this.scene
                .getParty()
                .filter((p) => p.isAllowedInBattle() && !p.isActive(true))
                .length
        ) {
            return super.end();
        }
        ...
        if (
            ...
            this.scene
                .getPlayerField()
                .filter((p) => p.isAllowedInBattle() && p.isActive(true))
                .length >= this.scene.currentBattle.getBattlerCount()
        ) {
            return super.end();
        }
...
export class LevelUpPhase extends PlayerPartyMemberPokemonPhase {
    ...
    start() {
        ...
        this.scene.validateAchvs(
            LevelAchv,
            new Utils.IntegerHolder(this.level)
        );
                    ...
                    () => {
                        this.scene.ui.showText(
                            ...
                            () => {
                                this.scene.ui.showText(
                                    i18next.t(
                                        "battle:learnMoveReplaceQuestion",
                                        {
                                            moveName: move.name,
                                        }
                                    ),
                                    ...
                                    () => {
                                        ...
                                        this.scene.ui.setModeWithoutClear(
                                            ...
                                            () => {
                                                ...
                                                this.scene.ui.showText(
                                                    i18next.t(
                                                        "battle:learnMoveForgetQuestion"
                                                    ),
                                                    null,
                                                    () => {
                                                        this.scene.ui.setModeWithoutClear(
    ...
    start() {
        ...
        if (healOrDamage) {
            ...
            if (pokemon.isPlayer()) {
                ...
                if (
                    healAmount.value > this.scene.gameData.gameStats.highestHeal
                ) {
                    this.scene.gameData.gameStats.highestHeal =
                        healAmount.value;
                }
            }
        ...
        if (this.healStatus && lastStatusEffect && !hasMessage) {
            this.scene.queueMessage(
                getPokemonMessage(
                    pokemon,
                    getStatusEffectHealText(lastStatusEffect)
                )
            );
        }
...
export class AttemptCapturePhase extends PokemonPhase {
    ...
    start() {
        ...
        const pokeballMultiplier = getPokeballCatchMultiplier(
            this.pokeballType
        );
        ...
        this.pokeball = this.scene.addFieldSprite(
            16,
            80,
            "pb",
            pokeballAtlasKey
        ...
        this.scene.tweens.add({
            ...
            onComplete: () => {
                ...
                this.scene.tweens.add({
                    ...
                    y: 20,
                    onComplete: () => {
                        this.pokeball.setTexture(
                            "pb",
                            `${pokeballAtlasKey}_opening`
                        );
                        pokemon.setVisible(false);
                            ...
                            const shakeCounter = this.scene.tweens.addCounter({
                                ...
                                onUpdate: (t) => {
                                    if (shakeCount && shakeCount < 4) {
                                        ...
                                        this.pokeball.setX(
                                            pbX +
                                                value * 4 * directionMultiplier
                                        );
                                        this.pokeball.setAngle(
                                            value * 27.5 * directionMultiplier
                                        );
                                    }
                                },
                                onRepeat: () => {
        ...
        this.scene.ui.showText(
            ...
            () => {
                ...
                const addToParty = () => {
                    ...
                    if (
                        this.scene.getParty().filter((p) => p.isShiny())
                            .length === 6
                    ) {
                        this.scene.validateAchv(achvs.SHINY_PARTY);
                    }
    ...
    start() {
        ...
        const modifierSelectCallback = (
            ...
            switch (rowCursor) {
                ...
                default:
                    ...
                    const shopOption =
                        shopOptions[
                            rowCursor > 2 ||
                            shopOptions.length <= SHOP_OPTIONS_ROW_LIMIT
                                ? cursor
            ...
            const applyModifier = (
                ...
                const result = this.scene.addModifier(
                    modifier,
                    false,
                    playSound
                );
            ...
            if (modifierType instanceof PokemonModifierType) {
                if (modifierType instanceof FusePokemonModifierType) {
                    ...
                    const pokemonModifierType =
                        modifierType as PokemonModifierType;
                    ...
                    this.scene.ui.setModeWithoutClear(
                        ...
                        modifierType instanceof PokemonMoveModifierType
                            ? (modifierType as PokemonMoveModifierType)
                                  .moveSelectFilter
                            : undefined,
                        tmMoveId,
                        isPpRestoreModifier
...
export class EggLapsePhase extends Phase {
    ...
    start() {
        ...
        const eggsToHatch: Egg[] = this.scene.gameData.eggs.filter(
            (egg: Egg) => {
        ...
        if (eggCount) {
            ...
            for (const egg of eggsToHatch) {
                this.scene.unshiftPhase(
                    new EggHatchPhase(this.scene, egg, eggCount)
                );
                if (eggCount > 0) {
                    eggCount--;
                }
