...
export default class PartyUiHandler extends MessageUiHandler {
    ...
    show(args: any[]): boolean {
        ...
        this.selectCallback =
            args.length > 2 && args[2] instanceof Function
                ? args[2]
                : undefined;
        this.selectFilter =
        ...
        if (this.optionsMode) {
            ...
            if (button === Button.ACTION) {
                ...
                if (
                    ...
                    const getTransferrableItemsFromPokemon = (
                        pokemon: PlayerPokemon
                    ) =>
                    ...
                    if (
                        ...
                        filterResult = (
                            this
                                .selectFilter as PokemonModifierTransferSelectFilter
                        )(
                            pokemon,
                        ...
                        if (
                            ...
                            if (option === PartyOption.TRANSFER) {
                                if (this.transferCursor !== this.cursor) {
                                    if (this.transferAll) {
                                        getTransferrableItemsFromPokemon(
                                            ...
                                            (
                                                this
                                                    .selectCallback as PartyModifierTransferSelectCallback
                                            )(
                                                this.transferCursor,
                                                i,
                                ...
                                if (option === PartyOption.SPLICE) {
                                    (
                                        this
                                            .selectCallback as PartyModifierSpliceSelectCallback
                            ...
                            if (
                                ...
                                this.scene.getCurrentPhase() instanceof
                                    SelectModifierPhase
                            ) {
                                if (this.partyUiMode === PartyUiMode.CHECK) {
                                    let formChangeItemModifiers =
                                        this.scene.findModifiers(
                                            (m) =>
                                                m instanceof
                                                    PokemonFormChangeItemModifier &&
                                    ...
                                    if (
                                        formChangeItemModifiers.find(
                                            (m) => m.active
                                        )
                                    ) {
                                        formChangeItemModifiers =
                                            formChangeItemModifiers.filter(
                                                (m) => m.active
                                            );
                                    }
                                    const modifier =
                ...
                if (this.partyUiMode === PartyUiMode.REMEMBER_MOVE_MODIFIER) {
                    ...
                    const pokemon = this.scene.getParty()[this.cursor];
                    const move =
                        allMoves[pokemon.getLearnableLevelMoves()[option]];
                    if (move) {
                    ...
                    if (
                        ...
                        const itemModifiers = this.scene.findModifiers(
                            (m) =>
                                ...
                                (m as PokemonHeldItemModifier).pokemonId ===
                                    this.scene.getParty()[this.cursor].id
                        ) as PokemonHeldItemModifier[];
                        this.transferQuantities = itemModifiers.map((item) =>
                            item.getStackCount()
                        );
                        this.transferQuantitiesMax = itemModifiers.map((item) =>
                            item.getStackCount()
                        );
                    }
                    this.showOptions();
    ...
    setCursor(cursor: integer): boolean {
        ...
        if (this.optionsMode) {
            ...
            if (changed && this.optionsScroll) {
                if (
                    Math.abs(cursor - this.optionsCursor) ===
                    this.options.length - 1
                ) {
                    this.optionsScrollCursor = cursor
                        ? this.optionsScrollTotal - 8
                        : 0;
                    this.updateOptions();
                } else {
        ...
        switch (this.partyUiMode) {
            ...
            case PartyUiMode.MODIFIER_TRANSFER:
                if (!this.transferMode) {
                    optionsMessage =
                        "Select a held item to transfer.\nUse < and > to change the quantity.";
                }
                break;
                        ...
                        if (
                            ...
                            this.scene.findModifier(
                                (m) =>
                                    ...
                                    (m as SwitchEffectTransferModifier)
                                        .pokemonId ===
                ...
                case PartyUiMode.CHECK:
                    if (
                        ...
                        if (formChangeItemModifiers.find((m) => m.active)) {
                            formChangeItemModifiers =
                                formChangeItemModifiers.filter((m) => m.active);
                        }
                        for (
                            let i = 0;
                            i < formChangeItemModifiers.length;
                            i++
                        ) {
            ...
            if (option === PartyOption.SCROLL_UP) {
                ...
                switch (option) {
                    ...
                    case PartyOption.MOVE_4:
                        const move =
                            pokemon.moveset[option - PartyOption.MOVE_1];
                        if (this.showMovePp) {
                    ...
                    default:
                        if (
                            ...
                            optionName = `${
                                modifier.active ? "Deactivate" : "Activate"
                            } ${modifier.type.name}`;
                        } else {
                            if (this.localizedOptions.includes(option)) {
                                optionName = i18next.t(
                                    `partyUiHandler:${PartyOption[option]}`
                                );
                            } else {
                                optionName = Utils.toReadableString(
                                    PartyOption[option]
                                );
                            }
                        }
    ...
    doRelease(slotIndex: integer): void {
        this.showText(
            ...
            () => {
                ...
                const releasedPokemon = this.scene
                    .getParty()
                    .splice(slotIndex, 1)[0];
...
class PartySlot extends Phaser.GameObjects.Container {
    ...
    setup(partyUiMode: PartyUiMode, tmMoveId: Moves) {
        ...
        const slotLevelLabel = this.scene.add.image(
            0,
            0,
            "party_slot_overlay_lv"
        );
        ...
        if (genderSymbol) {
            ...
            slotGenderText.setColor(
                getGenderColor(this.pokemon.getGender(true))
            );
            ...
            if (this.slotIndex >= battlerCount) {
                ...
                slotGenderText.setPositionRelative(
                    slotName,
                    76 - (this.pokemon.fusionSpecies ? 8 : 0),
                    3
                );
        ...
        if (this.pokemon.isShiny()) {
            ...
            if (doubleShiny) {
                ...
                fusionShinyStar.setTint(
                    getVariantTint(this.pokemon.fusionVariant)
                );
        ...
        if (partyUiMode !== PartyUiMode.TM_MODIFIER) {
            ...
            switch (true) {
                ...
                case this.pokemon
                    .getMoveset()
                    .filter((m) => m?.moveId === tmMoveId).length > 0:
                    slotTmText = "Learned";
                    break;
                default:
                    slotTmText = "Able";
