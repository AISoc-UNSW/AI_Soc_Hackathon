...
export const RunCommand = (scene: BattleScene) => {
    (scene.getCurrentPhase() as CommandPhase).handleCommand(Command.RUN, 0)
}

export const getMoveSet = (scene: BattleScene) => {
    const commandPhase = (scene.getCurrentPhase() as CommandPhase)
    const playerPokemon = scene.getPlayerPokemon()[commandPhase.getFieldIndex()]
    return playerPokemon.getMoveset()
}

export const getEnemy = (scene: BattleScene) => {
    console.log("helo world")
}

export const FightCommand = (scene: BattleScene, moveId: number) => {
    ...
    const moveIndex = playerPokemon.getMoveset().findIndex(m => m.moveId === moveQueue[0].move)
    commandScene.handleCommand(Command.FIGHT, moveIndex);
